...
import com.spms.vehicle.dto.response.ApiError;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleVehicleNotFound(VehicleNotFoundException ex, HttpServletRequest request) {
        return build(HttpStatus.NOT_FOUND, request, ex.getMessage());
    }

    @ExceptionHandler(DuplicateVehicleNumberException.class)
    public ResponseEntity<ApiError> handleDuplicateVehicleNumber(DuplicateVehicleNumberException ex,
            HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, request, ex.getMessage());
    }

    @ExceptionHandler(VehicleAlreadyInsideException.class)
    public ResponseEntity<ApiError> handleVehicleAlreadyInside(VehicleAlreadyInsideException ex,
            HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, request, ex.getMessage());
    }

    @ExceptionHandler(VehicleNotInsideException.class)
    public ResponseEntity<ApiError> handleVehicleNotInside(VehicleNotInsideException ex, HttpServletRequest request) {
        return build(HttpStatus.CONFLICT, request, ex.getMessage());
    }
